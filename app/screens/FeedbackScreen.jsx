          ...
          <View style={styles.ratingRow}>
            {RATINGS.map(r => (
              <TouchableOpacity key={r.value} style={[styles.ratingBtn, rating === r.value && styles.ratingBtnActive]} onPress={() => setRating(r.value)} activeOpacity={0.8}>
                ...
                <Text style={[styles.ratingLabel, rating === r.value && { color: "#fff", fontWeight: "800" }]}>{r.label}</Text>
              </TouchableOpacity>
            ))}
          </View>
          {rating > 0 && (
...
const styles = StyleSheet.create({
  ...
  ratingBtn: { flex: 1, backgroundColor: C.surface, borderRadius: 20, padding: 12, alignItems: "center", borderWidth: 2, borderColor: C.border, elevation: 3 },
  ratingBtnActive: { borderColor: C.navy, backgroundColor: C.navy, transform: [{ scale: 1.05 }] },
  ratingEmoji: { fontSize: 22, marginBottom: 4 },
  ratingLabel: { fontSize: 9, color: C.textMute, fontWeight: "700", textTransform: "uppercase", letterSpacing: 0.5 },
