...
const AdminNotifications = () => {
  ...
  // Fetch users & notifications on mount
  useEffect(() => {
  ...
  // Resolve user ID from notification message
  const resolveUserFromMessage = (message) => {
    ...
    let user = users.find((u) => {
      ...
      return (
        fullName1 === extractedName ||
        fullName2 === extractedName ||
    ...
    if (!user) {
      user = users.find((u) => {
        ...
        const username = normalize(u.username);
        return (
          fullName1.includes(extractedName) ||
          extractedName.includes(normalize(u.first_name)) ||
          username.includes(extractedName)
        );
      });
    }
