...
const DashboardContent = memo(() => {
  ...
  const handleClick = async (notif) => {
    ...
    // If this is an account registration notification or welcome message, just mark as read and don't navigate
    ...
    const navState = { requestId, autoOpen: true };

    if (message.includes('approved by the campus director')) {
      navigate(`/staffmaintenancerequestform/${requestId}`, { state: navState });
    } else if (message.includes('approved by the head')) {
      navigate(`/staffviewmaintenancerequestform/${requestId}`, { state: navState });
    } else if (message.includes('submitted')) {
      navigate(`/staffmaintenancerequestform/${requestId}`, { state: navState });
    } else {
      navigate(`/staffviewmaintenancerequestform/${requestId}`, { state: navState });
    }
