    ...
    try {
      ...
      if (result?.type === 'success') {
        ...
        if (idToken) {
          ...
          setError("Google Sign-In failed: no token received. Response: " + JSON.stringify(result));
        }
      } else if (result?.type !== 'cancel' && result?.type !== 'dismiss') {
        const errDetail = result?.error
          ? (typeof result.error === 'string' ? result.error : JSON.stringify(result.error))
          : result?.errorCode
            ? result.errorCode + ': ' + JSON.stringify(result?.params || {})
            : JSON.stringify(result);
        setError("Google Sign-In error: " + errDetail);
      }
    } catch (_err) {
      setError("Google Sign-In failed: " + (_err?.message || _err || "Please try again."));
    } finally {
