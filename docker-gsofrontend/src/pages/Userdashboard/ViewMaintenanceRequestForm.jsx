...
const handleLogout = async () => {
  ...
  const fieldsToDisplay = [
    ...
    { key: "scheduled_date", label: "Scheduled Date", category: "schedule" },
    { key: "scheduled_time", label: "Scheduled Time", category: "schedule" },
    { key: "scheduled_notes", label: "Scheduled Notes", category: "schedule" },
  ];
                  ...
                  <div className="bg-slate-50 border-t border-slate-200 px-6 py-4">
                    <div className="flex flex-col sm:flex-row gap-3 sm:justify-between sm:items-center">
                      ...
                      <div className="flex gap-3">
                        {(requestDetails.status?.toLowerCase().includes("completed") || 
                          requestDetails.status?.toLowerCase().includes("done")) && (
                          <button
                            onClick={() => navigate(`/userfeedback/${id}`)}
                            className="bg-green-600 hover:bg-green-700 text-white font-bold px-6 py-2 rounded-lg shadow transition-all active:scale-95 flex items-center gap-2"
                          >
