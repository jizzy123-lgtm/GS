...
=======
                      <form onSubmit={updateProfile}> {/*function na gi-call pag submit ang form*/}
                        {/* Profile Picture in Edit Mode */}
                        <div className="flex flex-col items-center justify-center mb-6">
                          <div className="relative">
                            {profilePicture ? ( // state variable nga nag-hold sa URL sa profile picture sa user
                              <img
                                src={profilePicture}  //* ipakita ang actual image gikan sa backend
                                alt="Profile"
                                className="w-24 h-24 rounded-full object-cover border-4 border-indigo-100 shadow-md"
                              />
                            ) : (
                              <div className="w-24 h-24 rounded-full bg-indigo-100 flex items-center justify-center border-4 border-indigo-200 shadow-md">
                                <span className="text-3xl text-indigo-500 font-medium">
                                  {editFormData.full_name ? editFormData.full_name.charAt(0).toUpperCase() : "U"}
                                </span>
                              </div>
                            )}
                            {/*camera button nga nag-trigger sa file picker para mag-upload ug profile picture.*/}
                            <button
                              type="button"
                              onClick={() => fileInputRef.current.click()} //* mag programmatically click sa hidden (file manager)
