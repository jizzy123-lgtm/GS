// [UPDATED 2026-03-28] Campus Director notifications enhancements (details modal + read/redirect behavior).
...
const DashboardContent = memo(() => {
  ...
  const handleNotificationClick = async (notif) => {
    ...
    if (notif.reference_id) {
      try {
        const response = await fetch(`${API_BASE_URL}/directorpov/${notif.reference_id}`, {
          headers: {
            Accept: 'application/json',
            ...(token ? { Authorization: `Bearer ${token}` } : {}),
          },
        ...
        >
          ...
          >
              ...
              {selectedNotif.type && (
                <div
                  className={`inline-block px-3 py-1 text-xs font-semibold rounded-full ${
                    selectedNotif.type === 'maintenance_request_verified'
                      ? 'bg-blue-100 text-blue-800'
                      : selectedNotif.type === 'maintenance_request_approved_by_head'
                        ? 'bg-purple-100 text-purple-800'
                        : selectedNotif.type === 'maintenance_request_approved_by_campus_director'
                          ? 'bg-green-100 text-green-800'
                          : 'bg-gray-100 text-gray-800'
                  }`}
                >
                  {selectedNotif.type === 'maintenance_request_verified' && 'Verified by Staff'}
                  {selectedNotif.type === 'maintenance_request_approved_by_head' && 'Approved by Head'}
                  {selectedNotif.type === 'maintenance_request_approved_by_campus_director' && 'Approved by Director'}
                  {!['maintenance_request_verified', 'maintenance_request_approved_by_head', 'maintenance_request_approved_by_campus_director'].includes(selectedNotif.type) && selectedNotif.type}
                </div>
              )}
            </div>
            ...
            {!loadingDetails && relatedData && (
              <div className="space-y-4">
                ...
                <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                  <h5 className="font-semibold text-gray-900 text-sm mb-3">Approval History</h5>
                  <div className="space-y-2 text-sm text-gray-700">
                    <p><span className="font-semibold">Verified by Staff:</span> {formatFieldValue(relatedData.verified_by)}</p>
                    <p><span className="font-semibold">Approved by Head:</span> {formatFieldValue(relatedData.approved_by_1)}</p>
                    <p><span className="font-semibold">Approved by Campus Director:</span> {formatFieldValue(relatedData.approved_by_2)}</p>
                  </div>
                </div>
            ...
            {selectedNotif.reference_id && (
              <button
                onClick={() => navigate(`/campusdirectormaintenancerequestform/${selectedNotif.reference_id}`)}
