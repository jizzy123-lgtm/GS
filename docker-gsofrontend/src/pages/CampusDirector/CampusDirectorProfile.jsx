...
} from '../../components/CampusDirectorSidebar';
...
const CampusDirectorProfile = () => {
  ...
  const uploadProfilePicture = async () => {
    ...
    try {
      ...
      setImgError(false);
      if (data.profile_picture) localStorage.setItem('cdProfilePicture', data.profile_picture);
  ...
  const updateProfile = async (e) => {
    ...
    if (!nameRegex.test(editFormData.first_name)) {
      setStatus((prev) => ({ ...prev, error: 'First Name can only contain letters, spaces, and hyphens.', success: null }));
      return;
    }
    if (!nameRegex.test(editFormData.last_name)) {
      setStatus((prev) => ({ ...prev, error: 'Last Name can only contain letters, spaces, and hyphens.', success: null }));
      return;
    }
  ...
  const viewInitial = fullName.charAt(0)?.toUpperCase() || 'C';
  const editInitial = editFormData.first_name?.charAt(0)?.toUpperCase()        || 'C';
                      ...
                      {!isEditing ? (
                        ...
                        <>
                          ...
                          <div className="mb-6">
                            ...
                            <p className="text-indigo-500 text-center font-medium mt-1">Campus Director</p>
                          </div>
                          ...
                          {/* ✅ Picture preview bar — same fix as Profile & StaffProfile */}
                          {previewImage && (
...
export default CampusDirectorProfile;