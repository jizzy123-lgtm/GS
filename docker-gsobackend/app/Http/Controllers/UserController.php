...
class UserController extends Controller
{
    ...
    public function getUsPass(){
        ...
        $Users = User::select('id', 'last_name','first_name', 'middle_name', 'suffix', 'username', 'password')
                            ->orderBy('created_at', 'desc')
                            ->get();
    ...
    public function updateProfile(Request $request)
    {
        ...
        try {
            ...
            if ($request->contact_number) {
                ...
                $raw = preg_replace('/^63/', '',  $raw);  // remove 63
                $raw = ltrim($raw, '+');                  // remove stray +
                if (!str_starts_with($raw, '0')) {
                    $raw = '0' . $raw;                   // add leading 0 → 09XXXXXXXXX
                }
                $request->merge(['contact_number' => $raw]);
            }

            $request->validate([
                'last_name'             => ['sometimes', 'string', 'min:1', 'max:255', 'regex:/^[a-zA-Z]+([\' -][a-zA-Z]+)*$/'],
                'first_name'            => ['sometimes', 'string', 'min:1', 'max:255', 'regex:/^[a-zA-Z]+([\' -][a-zA-Z]+)*$/'],
                'middle_name'           => ['nullable', 'string', 'regex:/^[a-zA-Z]$/', 'max:1'],
                'suffix'                => 'nullable|string|max:50',
                'contact_number'        => ['sometimes', 'nullable', 'regex:/^09[0-9]{9}$/'],
                ...
                'full_name'             => 'sometimes|nullable|string|min:1|max:255',
                'profile_picture'       => 'nullable|sometimes|image|mimes:jpg,jpeg,png,webp,gif|max:5120',
            ], [
                ...
                'full_name.min'         => 'Full name cannot be empty.',
                'contact_number.regex'  => 'Contact number must be 11 digits and start with 09.',
                'password.confirmed'    => 'Password confirmation does not match.',
                'password.min'          => 'Password must be at least 8 characters.',
                'password.max'          => 'Password cannot exceed 30 characters.',
            ...
            // ✅ contact_number is already normalized above, just save it
