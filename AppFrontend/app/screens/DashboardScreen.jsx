...
import { clearGoogleSession } from "../../hooks/useGoogleAuth";
    ...
    try {
      if (token) {
        ...
        if (!response.ok) {
          ...
          console.log("[DashboardScreen] backend logout failed:", response.status, responseText);
        }
      }
    } catch (error) {
      console.log("[DashboardScreen] backend logout error:", error?.message || error);
    } finally {
