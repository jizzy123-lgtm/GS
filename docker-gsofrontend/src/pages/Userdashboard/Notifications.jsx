...
// Detect whether a notification is maintenance-related
...
const MaintenanceModal = memo(({ request, onClose }) => {
  ...
  // Normalise priority number: accept numeric field or derive from label
  const PRIORITY_NUMBER = { high: 1, medium: 2, low: 3 };
  const priorityNum = request.priority_number
    ?? request.priority_level
    ?? PRIORITY_NUMBER[priorityKey]
    ?? null;
  ...
  const fields = [
    ...
    { label: 'Priority No.',  value: priorityNum !== null ? `P${priorityNum}` : null },
...
// ─── Priority chip for notification row ───────────────────────────────────────
const PRIORITY_NUM = { high: 1, medium: 2, low: 3 };

const PriorityChip = ({ priority }) => {
  if (!priority) return null;
  const key  = priority.toLowerCase();
  const meta = PRIORITY_META[key] || PRIORITY_META.low;
  const num  = PRIORITY_NUM[key]  || 3;
  return (
...
// ─── Type chip for notification row ───────────────────────────────────────────
const TypeChip = ({ type }) => {
  if (!type) return null;
...
const NotificationItem = memo(({ notif, onClickMaintenance }) => {
  ...
  // Pull priority / type from the notification or its embedded maintenance_request
  const priority = notif.priority
    || notif.maintenance_request?.priority
    || null;
  const type     = notif.type
    || notif.maintenance_request?.type
    || (isMaint ? 'maintenance_request' : null);
    ...
    >
      <div className="flex items-start gap-3 flex-1 min-w-0">
        ...
        <div className="min-w-0 flex-1">
          ...
          {/* ── Meta chips: type · priority · date received ── */}
          <div className="flex flex-wrap items-center gap-1.5 mt-2">
            {isMaint && priority && <PriorityChip priority={priority} />}
          </div>

          {isMaint && (
            <p className="mt-1.5 text-xs font-medium text-orange-500 flex items-center gap-1 group-hover:underline">
...
const DashboardContent = memo(() => {
  ...
  const handleMaintenanceClick = useCallback(async (notif) => {
    // Use embedded object if available
  ...
  return (
    <>
      ...
      {/* Loading overlay while fetching request details */}
